...
describe('Adapters', () => {
    ...
    describe('GitHubAdapter', () => {
        ...
        it('should fetch tasks without filter', async () => {
            (runCli as jest.Mock).mockResolvedValue(JSON.stringify([
                { number: 1, title: 'Task 1', state: 'OPEN', assignees: [{ login: 'john' }], updatedAt: '2023-01-01', labels: [{ name: 'bug' }], milestone: { title: 'Sprint 1' }, url: 'https://github.com/repo/issues/1' }
            ]));
            const tasks = await adapter.getTasks();
            expect(runCli).toHaveBeenCalledWith('gh', ['issue', 'list', '--json', 'number,state,title,assignees,updatedAt,labels,milestone,url']);
        ...
        it('should fetch tasks with filter', async () => {
            ...
            expect(runCli).toHaveBeenCalledWith('gh', ['issue', 'list', '--json', 'number,state,title,assignees,updatedAt,labels,milestone,url', '--search', 'updated:>=2023-01-01']);
        });
        ...
        it('should get a single task', async () => {
            ...
            expect(runCli).toHaveBeenCalledWith('gh', ['issue', 'view', '1', '--json', 'number,state,title,assignees,body,comments,updatedAt,labels,milestone,url']);
        ...
        it('should get current user', async () => {
            (runCli as jest.Mock).mockResolvedValue('dpk');
            const user = await adapter.getCurrentUser();
            expect(runCli).toHaveBeenCalledWith('gh', ['api', 'user', '--jq', '.login']);
            expect(user).toBe('dpk');
        });
    });
    ...
    describe('AzureAdapter', () => {
        ...
        it('should fetch tasks without filter', async () => {
            (runCli as jest.Mock).mockResolvedValue(JSON.stringify([
                { id: 1, fields: { 'System.Title': 'Task 1', 'System.State': 'Active', 'System.AssignedTo': { uniqueName: 'john' }, 'System.ChangedDate': '2023-01-01', 'System.Tags': 'bug', 'System.IterationPath': 'Sprint 1' } }
            ]));
            const tasks = await adapter.getTasks();
            expect(runCli).toHaveBeenCalledWith('az', ['boards', 'query', '--wiql', "SELECT [System.Id], [System.State], [System.Title], [System.AssignedTo], [System.ChangedDate], [System.Tags], [System.IterationPath] FROM workitems"]);
            expect(tasks).toHaveLength(1);
            expect(tasks[0]).toEqual({
                id: '1',
                title: 'Task 1',
        ...
        it('should get current user', async () => {
            (runCli as jest.Mock).mockResolvedValue('john@contoso.com');
            const user = await adapter.getCurrentUser();
            expect(runCli).toHaveBeenCalledWith('az', ['account', 'show', '--query', 'user.name', '-o', 'tsv']);
            expect(user).toBe('john@contoso.com');
        });
    });
});
